...
import { FiChevronLeft, FiChevronRight, FiClock, FiCheck, FiGrid, FiSave, FiCheckCircle } from 'react-icons/fi';
...
const ExamScreen = () => {
  ...
  const totalQuestions = examData?.groupedSubjects?.reduce(
   ...
   */
  const submitSingleAnswer = useCallback(async (questionId, optionId) => {
    try {
  ...
  useEffect(() => {
    ...
    const totalAnswered = examData.groupedSubjects
      ...
      .filter(q => selectedOptions[q.id] !== undefined || q.selected_option_id !== null)
      .length;
   ...
   */
  const navigateToQuestion = useCallback((subjectIdx, questionIdx) => {
  ...
  const subjectProgress = examData?.groupedSubjects?.map(group => {
        ...
        <div className="container mx-auto px-4 py-3">
          <div className="flex justify-between items-center">
            ...
            <div className={`flex items-center px-4 py-2 rounded-lg ${
              ...
              {timeLeft <= 300 && (
                <span className="ml-2 text-xs">HURRY!</span>
              )}
            </div>
          </div>
        </div>
              ...
              {examData.groupedSubjects.map((group, groupIdx) => {
                ...
                return (
                  <div key={groupIdx} className="border-b border-gray-100 last:border-0">
                    <h3 className={`px-4 py-2 font-medium ${
                      isEnglish ? 'bg-green-50 text-green-700' : 'bg-gray-50 text-blue-700'
                    }`}>
                      {group.subject} ({subjectProgress[groupIdx]?.answered || 0}/{group.questions.length})
                      {isEnglish && <span className="ml-2 text-xs text-green-600">(Compulsory)</span>}
                    </h3>
            ...
            >
              ...
              {saveStatus[currentQuestion.id] && (
                <div className={`mt-4 text-sm flex items-center ${
                  saveStatus[currentQuestion.id] === 'saving' ? 'text-blue-600' :
                  saveStatus[currentQuestion.id] === 'saved' ? 'text-green-600' :
                  'text-red-600'
                }`}>
